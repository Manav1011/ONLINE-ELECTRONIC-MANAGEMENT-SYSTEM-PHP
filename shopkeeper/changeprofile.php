...
$email="SELECT `SHOPKEEPER_EMAIL` FROM `shopkeeper_master` WHERE `SHOPKEEPER_ID`='1'";
$query= mysqli_query($con,$email);
 while($row=mysqli_fetch_array($query)){
 $a=$row['SHOPKEEPER_EMAIL'];
 }
 $password="SELECT `SHOPKEEPER_PASSWORD` FROM `shopkeeper_master` WHERE `SHOPKEEPER_ID`='1'";
$query1= mysqli_query($con,$password);
 while($row=mysqli_fetch_array($query1)){
 $b=$row['SHOPKEEPER_PASSWORD'];
 }
...
if(isset($_POST['submit'])){
    ...
    if($email1==$a){
      echo'<script>
       window.alert("Enter a different email");
       </script>';
    }
    elseif($ppassword!=$b){
      echo'<script>
    ...
    $update="UPDATE `shopkeeper_master` SET `SHOPKEEPER_EMAIL`='$email1',`SHOPKEEPER_PASSWORD`='$npassword' WHERE 1";
...
<font color="white">Log out</font>
  ...
  <div class="card-body">
      ...
      <h5><label for="exampleInputEmail1" class="form-label"> Change Email</label></h5>
          <input type="email" class="form-control" style="width:50%; margin-left:auto; margin-right:auto;" id="exampleInputEmail1" placeholder="Enter New Email...." name="email"  value="<?php $email2="SELECT SHOPKEEPER_EMAIL FROM `shopkeeper_master` WHERE SHOPKEEPER_ID='1'";
   $query= mysqli_query($con,$email2);
    while($row=mysqli_fetch_array($query)){
    echo $row['SHOPKEEPER_EMAIL'];
    }
    ?>" required>
          
          <br><h5><label for="exampleInputEmail1" class="form-label"> Change Password</label></h5>
          <input type="password" class="form-control" style="width:50%; margin-left:auto; margin-right:auto;" id="exampleInputEmail1" placeholder="Enter Previous Password...." name="ppassword" required>
          <br>
          <input type="password" class="form-control" style="width:50%; margin-left:auto; margin-right:auto;" id="exampleInputEmail1" placeholder="Enter New Password...." name="npassword" required>
          <br>
          <input type="password" class="form-control" style="width:50%; margin-left:auto; margin-right:auto;" id="exampleInputEmail1" placeholder="Confirm New Password...." name="cpassword" required>
          <br>
