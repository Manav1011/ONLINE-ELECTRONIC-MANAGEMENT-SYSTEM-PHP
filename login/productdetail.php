...
CONTACT: <?php echo $row['CONTACT'];?>
 ...
 <br><br><button class="btn btn-primary"> <a href="changeprofile.php?id=<?php echo $id;?>" class="text-white"style="text-decoration:none;">Edit profile</a></button>
  ...
  <?php 
        $select2="SELECT * FROM `product_master` WHERE `PRODUCT_ID`='$id2'";
        $query2=mysqli_query($con,$select2);
        while($row2=mysqli_fetch_array($query2)){
          $pid=$row2['PRODUCT_ID'];
         $bname=$row2['BRAND_NAME'];
         $mname=$row2['MODEL_NAME'];
         $features=$row2['FEATURES'];
         $image=$row2['IMAGE'];
         $price=$row2['PRICE'];
        }?>
    <main class="container">
      ...
      <div class="left-column">
        <img data-image="red" class="active" src="http://localhost/ONLINE%20ELECTRONIC%20MANAGEMENT%20SYSTEM/shopkeeper/productimages/<?php echo $image;?>" alt="">
      </div>
      ...
      <div class="right-column">
        ...
        <div class="product-price">
          ...
          <br>
          <a href="addtocartfromproductdetail.php?pid=<?php echo $id2;?>&cid=<?php echo $id;?>" onclick="return confirm('Do you want to add this item to cart?');" class="cart-btn">Add to cart</a>
          <a href="cartbybuynow.php?cid=<?php echo $id;?>&pid=<?php echo $id2;?>" class="cart-btn">Buy Now</a>
          <br><br>
     <form action="addtocompare.php?pid=<?php echo $id2;?>&cid=<?php echo $id;?>" method="post">
                        
                        <button class="btn btn-outline-success" type="submit" name="addtocomparepd" style="width:100%;">Add to compare</button>
                        </form>
        <p style="font-size:50px; font-family:'Roboto', sans-serif; font-weight: 300; margin-top:20px; ">Reviews <form action="addreview.php?id=<?php echo $id;?>&id2=<?php echo $id2;?>" method="post"><textarea  rows="5" name="review" cols="30" placeholder="Enter your review..."></textarea></p>
        <button type="submit" name="post" class="btn btn-primary" style="position:absolute; left: 68%;top: 68%;">Post</button></form>
       <?php
$selectreview="SELECT * FROM `review_master` WHERE `PRODUCT_ID`='$id2'";
$reviewquery=mysqli_query($con,$selectreview);
$numreview=mysqli_num_rows($reviewquery);
if($numreview==0){
  ?>
<div class="card text-black bg-light">
  <div class="card-body">
    No reviews are there for this product!! 
  </div>
</div>
<?php
}
  ...
  while($reviewrow=mysqli_fetch_array($reviewquery)){
    $review=$reviewrow['REVIEWS'];
    $customerid=$reviewrow['CUSTOMER_ID'];
?>
        <div class="card text-black bg-light">
  <div class="card-body">
    <?php
    $selectcustomer="SELECT  `CUSTOMER_USERNAME` FROM `customer_master` WHERE `CUSTOMER_ID`='$customerid'";
    $querycustomer=mysqli_query($con,$selectcustomer);
    while($rowcustomer=mysqli_fetch_array($querycustomer)){
     $username=$rowcustomer['CUSTOMER_USERNAME'];
    ?>
  <h5 class="card-title" style="text-align:justify;"><?php echo $username;?></h5>
  <?php
    }
    ?>
   <p class="card-text" style="text-align:justify;"> <?php echo $review;?></p>
  </div>
</div>
<?php
}
