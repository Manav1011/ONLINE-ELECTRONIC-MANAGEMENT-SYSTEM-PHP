...
if($pquantity==0){
    $select="SELECT `BRAND_NAME` FROM `product_master` WHERE `PRODUCT_ID`='$pid'";
$query1=mysqli_query($con,$select);
while($row=mysqli_fetch_array($query1)){
$bname=$row['BRAND_NAME'];
}
    echo'<div class="alert alert-danger" role="alert" style="margin:auto;">
    Product is out of stock<a href="brand.php?id2=$cid&name='.$bname.'" style="text-decoration: underline;
}">Go Back!!</a>
  </div>';
}
else{
